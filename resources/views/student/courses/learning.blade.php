...
<body class="font-poppins text-[#0A090B]">
    <section id="content">
        ...
        <form method="POST"
            action="{{ route('learning.course.answer.store', ['course' => $course, 'question' => $question]) }}"
            ...
            <div class="flex flex-col gap-[30px] max-w-[750px] w-full">
                ...
                @foreach ($question->course_answers as $answer)
                    <label for="{{ $answer->id }}"
                        ...
                        <input type="radio" name="answer_id" value="{{ $answer->id }}" id="{{ $answer->id }}"
                            class="hidden">
                    </label>
                @endforeach
